...
#define SDL3_MAX_VERTS 4096
#define SDL3_RECT_SEG 10
#define SDL3_ARC_SEG 36
...
typedef struct {
  ...
  sdl3_texture_t *textures; // Dynamic array.
...
static f32 sdl3_min(f32 a, f32 b) { return a < b ? a : b; }

static SDL_FColor sdl3_color(cheese_color_t color) {
  return (SDL_FColor){
      .r = (f32)((color >> 16) & 0xFF) / 255.0f,
      .g = (f32)((color >> 8) & 0xFF) / 255.0f,
      .b = (f32)((color >> 0) & 0xFF) / 255.0f,
      .a = (f32)((color >> 24) & 0xFF) / 255.0f,
  };
}
...
static void vert_push(sdl3_renderer_t *renderer, f32 x, f32 y, f32 u, f32 v,
                      SDL_FColor color) {
...
static void vert_flush(sdl3_renderer_t *renderer, SDL_Texture *texture) {
...
static cheese_corners_t sdl3_scale_radii(cheese_corners_t radius, f32 w,
                                         f32 h) {
  ...
  f32 sc = 1.0f;
  if (tl + tr > w && tl + tr > 0)
    sc = sdl3_min(sc, w / (tl + tr));
  if (bl + br > w && bl + br > 0)
    sc = sdl3_min(sc, w / (bl + br));
  if (tl + bl > h && tl + bl > 0)
    sc = sdl3_min(sc, h / (tl + bl));
  if (tr + br > h && tr + br > 0)
    sc = sdl3_min(sc, h / (tr + br));
...
static void sdl3_draw_rect_gradient(void *userdata, cheese_corners_t radius,
  ...
  SDL_FPoint pts[4 * (SDL3_RECT_SEG + 1) + 1];
  u32 n = 0;
  ...
  for (u32 k = 0; k < 4; k++) {
    ...
    for (u32 i = 0; i <= SDL3_RECT_SEG; i++) {
      f32 t = corners[k].a0 +
              (f32)i / (f32)SDL3_RECT_SEG * (corners[k].a1 - corners[k].a0);
...
static void sdl3_border_edge(sdl3_renderer_t *renderer, SDL_FColor c, f32 ox0,
                             f32 oy0, f32 ox1, f32 oy1, f32 ix0, f32 iy0,
                             f32 ix1, f32 iy1) {
  quad(renderer, c, ox0, oy0, ox1, oy1, ix1, iy1, ix0, iy0);
}

static void sdl3_border_arc(sdl3_renderer_t *renderer, SDL_FColor c, f32 ocx,
  ...
  for (u32 i = 0; i <= SDL3_RECT_SEG; i++) {
    f32 t = a0 + (f32)i / (f32)SDL3_RECT_SEG * (a1 - a0);
...
static void sdl3_draw_border(void *userdata, cheese_corners_t radius, f32 x,
  ...
  if (sides & CHEESE_SIDE_TOP)
    sdl3_border_edge(renderer, c, x + tl, y, x + w - tr, y, ix + itl, iy,
                     ix + iw - itr, iy);
  if (sides & (CHEESE_SIDE_TOP | CHEESE_SIDE_RIGHT))
    sdl3_border_arc(renderer, c, x + w - tr, y + tr, tr, ix + iw - itr,
                    iy + itr, itr, -0.5f * (f32)M_PI, 0.0f);
  if (sides & CHEESE_SIDE_RIGHT)
    sdl3_border_edge(renderer, c, x + w, y + tr, x + w, y + h - br, ix + iw,
                     iy + itr, ix + iw, iy + ih - ibr);
  if (sides & (CHEESE_SIDE_BOTTOM | CHEESE_SIDE_RIGHT))
    sdl3_border_arc(renderer, c, x + w - br, y + h - br, br, ix + iw - ibr,
                    iy + ih - ibr, ibr, 0.0f, 0.5f * (f32)M_PI);
  if (sides & CHEESE_SIDE_BOTTOM)
    sdl3_border_edge(renderer, c, x + w - br, y + h, x + bl, y + h,
                     ix + iw - ibr, iy + ih, ix + ibl, iy + ih);
  if (sides & (CHEESE_SIDE_BOTTOM | CHEESE_SIDE_LEFT))
    sdl3_border_arc(renderer, c, x + bl, y + h - bl, bl, ix + ibl,
                    iy + ih - ibl, ibl, 0.5f * (f32)M_PI, (f32)M_PI);
  if (sides & CHEESE_SIDE_LEFT)
    sdl3_border_edge(renderer, c, x, y + h - bl, x, y + tl, ix, iy + ih - ibl,
                     ix, iy + itl);
  if (sides & (CHEESE_SIDE_LEFT | CHEESE_SIDE_TOP))
    sdl3_border_arc(renderer, c, x + tl, y + tl, tl, ix + itl, iy + itl, itl,
...
static void sdl3_draw_texture(void *userdata, f32 x, f32 y, f32 w, f32 h,
                              u32 texture_id, cheese_color_t color) {
  sdl3_renderer_t *renderer = (sdl3_renderer_t *)userdata;
  sdl3_texture_t *entry = sdl3_find_texture(renderer, texture_id);
  if (!entry)
    return;

  SDL_FColor c = sdl3_color(color);
  SDL_SetTextureColorModFloat(entry->texture, c.r, c.g, c.b);
  SDL_SetTextureAlphaModFloat(entry->texture, c.a);

  SDL_FRect src = {0, 0, (f32)entry->width, (f32)entry->height};
  SDL_FRect dst = {x, y, w, h};
  SDL_RenderTexture(renderer->renderer, entry->texture, &src, &dst);
}
...
static void sdl3_draw_line(void *userdata, f32 x1, f32 y1, f32 x2, f32 y2,
...
static void sdl3_draw_arc(void *userdata, f32 cx, f32 cy, f32 radius,
  ...
  u32 segments = (u32)(range / (2.0f * (f32)M_PI) * (f32)SDL3_ARC_SEG);
  if (segments < 3)
    segments = 3;
...
static void sdl3_emit_glyph(void *userdata, const cheese_glyph_quad_t *q) {
  sdl3_renderer_t *renderer = (sdl3_renderer_t *)userdata;
  sdl3_texture_t *entry = sdl3_find_texture(renderer, q->texture_id);
  if (!entry)
    return;

  renderer->pending_texture = entry->texture;
  SDL_FColor c = sdl3_color(q->color);
...
static void sdl3_draw_text(void *userdata, f32 x, f32 y, const string *text,
                           cheese_font_t *font, cheese_color_t color,
                           f32 scale) {
  sdl3_renderer_t *renderer = (sdl3_renderer_t *)userdata;
  renderer->pending_texture = null;

  cheese_font_shape_run(font, text, x, y, scale, color, sdl3_emit_glyph,
                        userdata);
...
static void sdl3_push_clip(void *userdata, f32 x, f32 y, f32 w, f32 h) {
  sdl3_renderer_t *renderer = (sdl3_renderer_t *)userdata;
  ...
  SDL_SetRenderClipRect(renderer->renderer, &rect);
}

static void sdl3_pop_clip(void *userdata) {
  sdl3_renderer_t *renderer = (sdl3_renderer_t *)userdata;
  if (renderer->clip_depth == 0)
    return;
  ...
  if (renderer->clip_depth > 0) {
    SDL_Rect parent = renderer->clip_stack[renderer->clip_depth - 1];
    SDL_SetRenderClipRect(renderer->renderer, &parent);
  } else {
...
static i32 sdl3_create_texture(void *userdata, u32 width, u32 height,
                               cheese_texture_format_t format, const u8 *data) {
  sdl3_renderer_t *renderer = (sdl3_renderer_t *)userdata;
  if (format != CHEESE_TEXTURE_RGBA8) {
    cheese_log_error("sdl3: only RGBA8 textures are supported");
    return 0;
  }
  ...
  if (!texture) {
    cheese_log_error("sdl3: SDL_CreateTexture failed: %s", SDL_GetError());
    return 0;
  }
  ...
  if (renderer->texture_count >= renderer->texture_cap) {
    ...
    sdl3_texture_t *grown = arena_alloc(renderer->arena, sdl3_texture_t, cap);
  ...
  renderer->textures[renderer->texture_count++] =
      (sdl3_texture_t){id, texture, width, height};
  return id;
}

static void sdl3_delete_texture(void *userdata, u32 texture_id) {
  sdl3_renderer_t *renderer = (sdl3_renderer_t *)userdata;
  for (u32 i = 0; i < renderer->texture_count; i++) {
    if ((u32)renderer->textures[i].id != texture_id)
      continue;
...
static void sdl3_update_texture_region(void *userdata, u32 texture_id, i32 x,
...
static void sdl3_flush_deferred(void *userdata) { (void)userdata; }

//
//
//

static void sdl3_flush_draws(void *userdata) { (void)userdata; }
...
cheese_renderer_t cheese_create_sdl3_renderer(SDL_Renderer *renderer,
                                              arena_t *arena) {
  sdl3_renderer_t *r = arena_alloc_zeroed(arena, sdl3_renderer_t, 1);
  r->renderer = renderer;
  r->arena = arena;
  r->next_texture_id = 1;
  r->vert_cap = SDL3_MAX_VERTS;
  ...
  result.draw_arc = sdl3_draw_arc;
  result.draw_text = sdl3_draw_text;
  result.push_clip = sdl3_push_clip;
  result.pop_clip = sdl3_pop_clip;
  result.create_texture = sdl3_create_texture;
  result.delete_texture = sdl3_delete_texture;
  result.update_texture_region = sdl3_update_texture_region;
  result.flush_deferred = sdl3_flush_deferred;
  result.flush_draws = sdl3_flush_draws;
