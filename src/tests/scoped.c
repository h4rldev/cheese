...
int main(void) {
  ...
  u32 prop =
      cheese_style_prop_register(&cheese, "scoped/test", CHEESE_PROP_F32);

  cheese_style_t def = cheese_style_new();
  cheese_style_set_bg_color(&def, 0x11);
  cheese_style_set_text_color(&def, 0x99);
  cheese_style_prop_set_f32(&cheese, &def, prop, 1.0f);
  ...
  cheese_style_prop_set_f32(&cheese, &scope, prop, 2.0f);
  cheese_push_style(&cheese, scope);

  cheese_style_t cls = cheese_style_new();
  cheese_style_prop_set_f32(&cheese, &cls, prop, 3.0f);
  ...
  cheese_style_prop_set_f32(&cheese, &explicit, prop, 4.0f);
  ...
  assert(cheese_style_prop_get_f32(&out, prop, -1.0f) == 4.0f &&
         "explicit wins the property");

  cheese_style_resolve_scoped(&cheese, &out, CHEESE_ROLE_SLIDER, "test-class",
                              null);
  assert(cheese_style_prop_get_f32(&out, prop, -1.0f) == 3.0f &&
  ...
  assert(cheese_style_prop_get_f32(&out, prop, -1.0f) == 2.0f &&
